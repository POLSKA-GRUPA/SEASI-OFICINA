...
| `oficina/` | event store humano local: fichaje, diario, tareas (JSONL append-only + hash-chain) · merge idempotente multi-máquina (`uid`+`origin`) | cadena/tamper + reglas fail-closed + proyecciones + sync E2E contra relay real |
| `relay/` (server) | tiempo real entre máquinas: salas por tenant, fan-out, replay, presencia — sin verdad de negocio | auth fail-closed, aislamiento entre tenants, replay, roster |
